...
6. If there are correctable mistakes worth tracking, save them: `node "${CLAUDE_PLUGIN_ROOT}/scripts/progress.mjs" add-session --data-dir "${CLAUDE_PLUGIN_DATA}" --scenario "passive-review" --level "<rough CEFR estimate>" --mistakes '<JSON array>'` — this feeds the same recurring-mistakes tracking used by `/devspeak:progresso`.
